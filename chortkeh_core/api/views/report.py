...
from chortkeh_core.models import Income, Expense, Transfer
...
class ReportExpenseApiView(views.APIView):
    """ This api view for get report of expense transaction with filters. """
...
class ReportTransferApiView(views.APIView):
    """ This api view for get report of transfer transaction with filters. """
    ...
    def get(self, request, *args, **kwargs):
        ...
        if filter_by == 'source' and filter_id is not None:
            tsf_obj = Transfer.objects.filter(
                source_wallet__owner=request.user,
                time__gte=time_gte, time__lte=time_lte,
                source_wallet_id=int(filter_id)
            ).order_by('-time')
        elif filter_by == 'target' and filter_id is not None:
            tsf_obj = Expense.objects.filter(
                target_wallet__owner=request.user,
                time__gte=time_gte, time__lte=time_lte,
                target_wallet_id=int(filter_id)
            ).order_by('-time')
        else:
            tsf_obj = Expense.objects.filter(
                source_wallet__owner=request.user,
        ...
        paginator = PaginationClass()
        paginate_results = paginator.paginate_queryset(tsf_obj, request)
        response_list = [{
            'id': q.id,
            'amount': q.amount,
            'time': utils.jdate_to_str(q.time),
            'comment': q.comment,
            'source_wallet_id': q.source_wallet_id,
            'source_wallet_name': q.source_wallet.name,
            'target_wallet_id': q.target_wallet_id,
            'target_wallet_name': q.target_wallet_name.name
        } for q in paginate_results]
        response_data = {
            'count': paginator.count,
            'next': paginator.get_next_link(),
